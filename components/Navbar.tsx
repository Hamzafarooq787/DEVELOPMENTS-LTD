  ...
  return (
    <>
          ...
          <div className="hidden md:flex gap-6 lg:gap-8 items-center">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`font-semibold text-sm uppercase transition-colors ${isActive(link.href)
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 h-full w-80 bg-[#111111] shadow-2xl border-l border-[#2c2c2c] transform transition-transform duration-300 ease-out ${isOpen ? "translate-x-0" : "translate-x-full"
            }`}
        >
          <div className="flex flex-col pt-24 pb-8 px-6 gap-5">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`font-semibold text-base uppercase py-2 transition-colors ${isActive(link.href)
